...
def load_topology(topology, system=None, xyz=None, box=None, condense_atom_types=True):
    ...
    for c in topology.chains():
        ...
        for r in c.residues():
            ...
            if residue in ['WAT', 'HOH', 'TP4', 'TP5', 'T4E']: # OpenMM waters are called HOH, but include other possibilities just in case
                if xyz is None: # We need coordinates to calculate the correct bond lengths
                    raise RuntimeError('Failed to convert OpenMM waters to AMBER format - no coordinates provided')
                # Note that the water atom positions must be correct and virtual sites MUST have been added by addExtraParticles (don't use deprecated function convertWater) for their coordinates to be correct
                if isinstance(xyz, str):
                    xyz = load_file(xyz, skip_bonds=True)
                    coordinates = xyz.coordinates
                else: # We pass the coordinates directly in Flare
                    coordinates = xyz
                ...
                for water_atom in r.atoms():
                    ...
                    if element is None:
                      ep_atoms.append(water_atom)
                    elif element and element.atomic_number == 8:
                ...
                if len(o_atoms) == 1 and len(h_atoms) == 2: # Definitely a water
                    ...
                    for ep in ep_atoms:
                        ep.name = 'EP' # Must be called EP or LP for the virtual sites to be recognised by OpenMM or AMBER when reading the prmtop file
                        extra_point_pos = coordinates[ep.index]
                        ...
                        bond_force.addBond(o_atoms[0].index, ep.index, o_ex_distance_nm, bond_force_constant_kj_mol_nm2)
